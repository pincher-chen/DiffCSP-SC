...
class GraphAttentionTransformer(torch.nn.Module):
    ...
    #def forward(self, f_in, pos, edge_src, edge_dest, edge_vec, edge_distances,
    #def forward(self, f_in, edge_src,edge_dst, edge_vec, edge_attr, edge_num,
    def forward(self, edge_src,edge_dst, edge_vec, edge_attr,
                batch, node_atom, **kwargs) -> torch.Tensor:
        ...
        #lattice_sh = o3.spherical_harmonics(l=self.irreps_edge_attr,
        #    x=lattices, normalize=True, normalization='component')
        #node_atom = node_atom.new_tensor([-1, 0, -1, -1, -1, -1, 1, 2, 3, 4])[node_atom]
        #print(lattice_sh.shape)
        atom_embedding, atom_attr, atom_onehot = self.atom_embed(node_atom)
        #print(atom_embedding.shape)
        #print(self.max_radius)
        edge_length = edge_attr #edge_vec.norm(dim=1)
        edge_length_embedding = self.rbf(edge_length)
        #print(edge_length_embedding.shape)
        #edge_length_embedding = self.rbf(edge_length, node_atom, edge_src, edge_dst)
        #print(edge_length_embedding)
        #exit()
        edge_degree_embedding = self.edge_deg_embed(atom_embedding, edge_sh, 
            edge_length_embedding, edge_src, edge_dst, batch)

	#add crystalnet_fea
        #data_zero = torch.zeros(edge_degree_embedding.shape[0], edge_degree_embedding.shape[1]-f_in.shape[1],device=f_in.device)
        #crystalnet_fea = torch.cat((f_in, data_zero),dim=1)

        #node_features = atom_embedding + edge_degree_embedding + crystalnet_fea.float()
        node_features = atom_embedding + edge_degree_embedding 
        #print(atom_embedding.shape)
        #print(edge_degree_embedding.shape)
        #print(edge_length_embedding.pow(2).mean())
        #print(node_features.shape)
        #exit()
        #node_features = f_in
        #print(node_features)
        node_attr = torch.ones_like(node_features.narrow(1, 0, 1))
        #print(node_features.narrow(1, 0, 1))
        #print(node_attr)
        #exit()
        #node_attr = torch.ones_like(atom_embedding.narrow(1, 0, 1))
...
def graph_attention_transformer_nonlinear_l2_e3(irreps_in, radius, num_basis=128, 
    ...
    model = GraphAttentionTransformer(
        ...
        #irreps_node_embedding='256x0e+64x0o+64x1e+64x1o', num_layers=6,
