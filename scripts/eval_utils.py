...
from diffcsp.common.data_utils import StandardScaler, chemical_symbols, lattice_params_to_matrix_torch, radius_graph_pbc, repeat_blocks
from diffcsp.pl_data.dataset import TensorCrystDataset
from diffcsp.pl_data.datamodule import worker_init_fn
#add
#from diffcsp.pl_modules.model import CrystGNN_Supervise
...
def reorder_symmetric_edges(
    edge_index, cell_offsets, neighbors, edge_vector
):
    """
...
def gen_edges(num_atoms, frac_coords, lattices, node2graph, edge_style):
    ...
    if edge_style == 'fc':
        ...
        edge_index, to_jimages, num_bonds = radius_graph_pbc(
            cart_coords, None, None, num_atoms, 6, 12,
        ...
        edge_index_new, _, _, edge_vector_new = reorder_symmetric_edges(edge_index, to_jimages, num_bonds, distance_vectors)

        return edge_index_new, -edge_vector_new

#end add code for gen_edges
...
def load_model(model_path, load_data=False, testing=True):
    with initialize_config_dir(str(model_path)):
        ...
        if os.path.exists(os.path.join(model_path,'prop_scaler.pt' )):
...
def structure_validity(crystal, cutoff=0.5, cutoff_max=5.0):
    dist_mat = crystal.distance_matrix
    # Pad diagonal with a large number
    dist_mat = dist_mat + np.diag(
        np.ones(dist_mat.shape[0]) * (cutoff + 10.))
    # if dist_mat.min() < cutoff or crystal.volume < 0.1 or max(crystal.lattice.abc) > 40:
    if dist_mat.min() < cutoff or dist_mat.min() > cutoff_max:
        return False
    else:
        return True
...
def prop_model_eval(eval_model_name, crystal_array_list, model_path = None):
    ...
    model_p = create_model(irreps_in='100x0e',radius=8,num_basis=128,out_channels=1,atomref=None,drop_path=0.0)
    ...
    for batch in loader:
       ...
       # print(preds)
        ...
        edges_index, edge_vec = gen_edges(batch.num_atoms, batch.frac_coords, lattices, node2graph, edge_style='knn')
        preds = model_p(batch=node2graph,node_atom=batch.atom_types, edge_src=edges_index[1], edge_dst=edges_index[0], edge_attr=edge_vec.norm(dim=1), edge_vec=edge_vec)
        print(preds)
#        exit()
