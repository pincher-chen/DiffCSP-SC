...
class PropEnsemble(object):

    def __init__(self, pred_crys, gt_crys, gt_props, eval_model_name=None):
        # assert len(pred_crys) == len(gt_crys)
        self.preds_all = pred_crys
        self.gts = gt_crys
        self.batch_size = len(self.preds_all)
        self.gt_props = gt_props
        self.eval_model_name = eval_model_name
        self.get_props()
        self.get_props_upper()

    def get_props(self):
        props = np.zeros((self.batch_size, len(self.preds_all[0])))
        masks = np.zeros((self.batch_size, len(self.preds_all[0])))
        for i in range(self.batch_size):
            print(f"Property Prediction at Batch {i}")
            sample_i = self.preds_all[i]
            valid_sample_i = [c for c in sample_i if c.valid]
            prop_i = prop_model_eval(self.eval_model_name, [c.dict for c in valid_sample_i])
            mask = np.array([i for (i,c) in enumerate(sample_i) if c.valid])
            props[i, mask] = prop_i
            masks[i, mask] = 1.
        self.props_all = props
        self.props = props.sum(axis=0) / masks.sum(axis=0)
    
    def get_props_upper(self):
        gt_props = prop_model_eval(self.eval_model_name, [c.dict for c in self.gts])
        self.props_upper = np.array(gt_props)

    def cal_metric(self, pred, gt, prefix):
        mask = (pred == pred) & (gt == gt)
        pred, gt = pred[mask], gt[mask]
        mae = np.abs(pred - gt).mean()
        pcc = np.corrcoef(pred, gt)[0][1]
        smape = np.mean(np.abs(pred - gt) / ((np.abs(gt) + np.abs(pred)) / 2))
        return {f'{prefix}_MAE':mae, f'{prefix}_sMAPE':smape, f'{prefix}_PCC': pcc}

    def get_metrics(self):
        metrics = {}
        metrics.update(self.cal_metric(self.props, self.gt_props, 'Pred'))
        metrics.update(self.cal_metric(self.props_upper, self.gt_props, 'GT'))
        return metrics
...
def main(args):
    ...
    if 'gen' in args.tasks:
        ...
        if 'prop' in args.tasks:

            csv = pd.read_csv(args.gt_file)
            gt_crys = p_map(get_gt_crys_ori, csv['cif'])
            prop_list = csv[cfg.data.prop]

            prop_evaluator = PropEnsemble(pred_crys, gt_crys, prop_list, eval_model_name)

            prop_metrics = prop_evaluator.get_metrics()
            all_metrics.update(prop_metrics)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tasks', nargs='+', default=['struct','prop', 'gen'])
